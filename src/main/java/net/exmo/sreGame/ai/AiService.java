...
/**
 * LLM HTTP 客户端，移植自源工程 {@code cn.lanink.situationpuzzlegame.ai.AiService}。
 * 已去除 Nukkit 依赖与 Lombok，改为接收组装好的 system/user 提示词，便于复用。
 * 支持 OpenAI 与 Anthropic 两种协议，以及 OpenCode GO 端点自动路由。
 */
public final class AiService {
   ...
   private static final Set<String> OPENCODE_GO_OPENAI_MODELS =
         Set.of("glm-5.1", "glm-5", "kimi-k2.5", "kimi-k2.6",
   ...
   public CompletableFuture<GenerateResult> generatePuzzle(String systemPrompt, String userPrompt) {
      return generatePuzzle(this.config.generatorProvider(), systemPrompt, userPrompt);
   }

   public CompletableFuture<GenerateResult> generatePuzzle(AiProviderConfig provider, String systemPrompt, String userPrompt) {
      try {
         AiProviderConfig p = requireProvider(provider, "generator");
         String requestApiType = resolveRequestApiType(p);
         JsonObject body = new JsonObject();
         body.addProperty("model", normalizeModel(p.model()));
         applySystemAndMessages(body, systemPrompt, userPrompt, requestApiType);
         applyThinking(body, p.thinkingType(), p.reasoningEffort(), requestApiType,
               isOpenCodeGoConfig(p));
         applyMaxTokens(body, requestApiType);
         return callApi(p.apiUrl(), p.apiKey(), body, requestApiType, isOpenCodeGoConfig(p))
               .orTimeout(120, TimeUnit.SECONDS)
               .thenApply(response -> parseGenerateResponse(response))
   ...
   public CompletableFuture<AnswerType> answerQuestion(String systemPrompt, String userPrompt) {
      return answerQuestion(this.config.answererProvider(), systemPrompt, userPrompt);
   }

   public CompletableFuture<AnswerType> answerQuestion(AiProviderConfig provider, String systemPrompt, String userPrompt) {
      try {
         AiProviderConfig p = requireProvider(provider, "answerer");
         String requestApiType = resolveRequestApiType(p);
         JsonObject body = new JsonObject();
         body.addProperty("model", normalizeModel(p.model()));
         applySystemAndMessages(body, systemPrompt, userPrompt, requestApiType);
         ...
         return callApi(p.apiUrl(), p.apiKey(), body, requestApiType, isOpenCodeGoConfig(p))
               ...
               .thenApply(response -> parseAnswerResponse(response));
      } catch (Exception e) {
         return CompletableFuture.failedFuture(e);
      }
   }

   public CompletableFuture<SoloAnswerResult> answerSoloQuestion(String systemPrompt, String userPrompt) {
      return answerSoloQuestion(this.config.answererProvider(), systemPrompt, userPrompt);
   }

   public CompletableFuture<SoloAnswerResult> answerSoloQuestion(AiProviderConfig provider, String systemPrompt, String userPrompt) {
      try {
         AiProviderConfig p = requireProvider(provider, "answerer");
         String requestApiType = resolveRequestApiType(p);
         JsonObject body = new JsonObject();
         ...
         return callApi(p.apiUrl(), p.apiKey(), body, requestApiType, isOpenCodeGoConfig(p))
               .orTimeout(60, TimeUnit.SECONDS)
               .thenApply(response -> parseSoloAnswerResponse(response));
      } catch (Exception e) {
         return CompletableFuture.failedFuture(e);
      }
   }
   ...
   private void applyThinking(JsonObject body, String thinkingType, String reasoningEffort,
      ...
      if (thinkingType == null || "disabled".equals(thinkingType)) {
         return;
      }
   ...
   private GenerateResult parseGenerateResponse(String responseBody) {
      try {
   ...
   private AnswerType parseAnswerResponse(String responseBody) {
      String content = extractContent(responseBody).trim();
      return parseAnswerType(content);
   }

   private SoloAnswerResult parseSoloAnswerResponse(String responseBody) {
      String content = extractContent(responseBody).trim();
      String jsonStr = extractJson(content);
      JsonObject parsed = JsonParser.parseString(jsonStr).getAsJsonObject();
      AnswerType answerType = parseAnswerType(parsed.get("answer").getAsString());
      boolean solved = parsed.has("solved") && parsed.get("solved").getAsBoolean();
      return new SoloAnswerResult(answerType, solved);
   }

   static AnswerType parseAnswerType(String content) {
      ...
      if (normalized.contains("irrelevant")
         ...
         return AnswerType.IRRELEVANT;
      }
      if (normalized.contains("yesorno")
            || normalized.contains("maybe")
            || content.contains("是或不是")
            || content.contains("是或者不是")) {
         return AnswerType.YES_OR_NO;
      }
      if (normalized.contains("no") || content.contains("不是") || content.contains("否")) {
         return AnswerType.NO;
      }
      if (normalized.contains("yes") || content.contains("是")) {
   ...
   private String resolveRequestApiType(AiProviderConfig provider) {
      String model = normalizeModel(provider.model());
      String url = provider.apiUrl();
      boolean openCodeGo = isOpenCodeGoConfig(provider);
      if (isMessagesEndpoint(url)) {
         return API_TYPE_ANTHROPIC;
      }
      if (isChatCompletionsEndpoint(url)) {
         return API_TYPE_OPENAI;
      }
      if (openCodeGo) {
         String modelKey = model.toLowerCase(Locale.ROOT);
         if (OPENCODE_GO_ANTHROPIC_MODELS.contains(modelKey)) {
            return API_TYPE_ANTHROPIC;
         }
         if (OPENCODE_GO_OPENAI_MODELS.contains(modelKey)) {
            return API_TYPE_OPENAI;
         }
      }
      return normalizeApiType(provider.apiType());
   }
   ...
   private boolean isOpenCodeGoConfig(AiProviderConfig provider) {
      return (provider.model() != null && provider.model().trim().startsWith(OPENCODE_GO_MODEL_PREFIX))
            || (provider.apiUrl() != null && provider.apiUrl().contains("opencode.ai/zen/go/v1"));
   }
   ...
   private AiProviderConfig requireProvider(AiProviderConfig provider, String role) {
      ...
      if (!provider.hasApiKey()) {
         throw new IllegalStateException(role + " 提供商未配置 API key: " + provider.name());
      }
      return provider;
   }
   ...
   public record SoloAnswerResult(AnswerType answerType, boolean solved) {
   }
}
