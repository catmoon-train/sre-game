...
public final class DrawGuessMiniGame implements net.exmo.sreGame.game.MiniGame {
   public static final String ID = "draw_guess";
   private final GameContext ctx;

   public DrawGuessMiniGame(GameContext ctx) {
      this.ctx = ctx;
   }
   ...
   public boolean canStart(GameRoom room, ServerPlayer actor) {
      ...
      if (room.size() < this.minPlayers() || room.size() > this.maxPlayers()) {
         this.ctx.send(actor, "&c你画我猜需要 &f" + this.minPlayers() + "–" + this.maxPlayers() + " &c人。");
         return false;
      }
      if (!room.allReady()) {
   ...
   public void start(GameRoom room, ServerPlayer actor) {
      ...
      if (matchId == null) {
         this.ctx.send(actor, "&c没有足够的画布场地。");
      }
   }
}
