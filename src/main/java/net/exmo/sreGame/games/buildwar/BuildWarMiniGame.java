...
public final class BuildWarMiniGame implements net.exmo.sreGame.game.MiniGame {
   public static final String ID = "build_war";
   private final GameContext ctx;

   public BuildWarMiniGame(GameContext ctx) {
      this.ctx = ctx;
   }
   ...
   public boolean canStart(GameRoom room, ServerPlayer actor) {
      ...
      if (room.size() < this.minPlayers() || room.size() > this.maxPlayers()) {
         this.ctx.send(actor, "&c建筑战争需要 &f" + this.minPlayers() + "–" + this.maxPlayers() + " &c人（当前 &f" + room.size() + "&c）。");
         return false;
      }
      if (!room.allReady()) {
